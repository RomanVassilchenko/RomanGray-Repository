...
def userRights(message):
    ...
    global NumberClass;
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True,one_time_keyboard=True)
    keyboard.add(*[types.KeyboardButton(name) for name in ["8", "9", "10", "11"]])
    bot.send_message(message.chat.id,
...
def Eighth_Grade(message):     #функция создающая ссылку на сайт с информацией (доделать сайт)
    NumberClass += '8';
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    keyboard.add(*[types.KeyboardButton(name) for name in ["A", "B", "C", "D"]])
    bot.send_message(message.chat.id,
                     "Выберите букву класса", reply_markup=keyboard)
...
def D_Grade(message):
    NumberClass += 'D';
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    keyboard.add(*[types.KeyboardButton(name) for name in ["Понедельник", "Вторник", "Среда", "Четверг"]])
    bot.send_message(message.chat.id,
                     "Выберите день недели", reply_markup=keyboard)
    bot.register_next_step_handler(message, DayChoose)


def DayChoose(message):
    if NumberClass == '8D':
