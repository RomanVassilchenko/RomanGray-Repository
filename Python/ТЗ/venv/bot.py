...
def start(message):
    bot.send_message(message.chat.id, get_from_DB("SELECT * FROM text_from_user")[0][1])
    bot.send_message(message.chat.id, get_from_DB("SELECT * FROM text_from_user")[1][1])
    bot.register_next_step_handler(message, start)
    global listRoma
    listRoma.append(message.text)
    bot.send_message(message.chat.id, get_from_DB("SELECT * FROM text_from_user")[2][1])
    bot.register_next_step_handler(message, start)
    listRoma.append(message.text)
    listRoma.append(0)
    listRoma.append( message.chat.id )
    get_num(message)


def get_num(message):
    bot.send_message(message.chat.id, get_from_DB("SELECT * FROM text_from_user")[3][1])
